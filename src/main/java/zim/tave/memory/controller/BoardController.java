...
public class BoardController {
    ...
    public ResponseEntity<ApiResponseDto<BoardCreateResponseDto>> createBoard(
        ...
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(ApiResponseDto.success(ResponseCode.BOARD_CREATE_SUCCESS, response));
    }
    ...
    @Operation(
            ...
            description = "보드의 기본 정보와 배치된 스티커 정보를 모두 조회합니다."
    )
    @ApiErrorCodeExamples({
            ErrorCode.AUTHENTICATION_FAILED,
            ErrorCode.INVALID_TOKEN,
            ErrorCode.UNAUTHORIZED_USER,
            ErrorCode.BOARD_ACCESS_FORBIDDEN,
            ErrorCode.BOARD_NOT_FOUND,
    })
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "보드 상세 조회 성공"),
            ...
            @ApiResponse(responseCode = "403", description = """
                접근 권한이 없습니다.
                - BOARD_ACCESS_FORBIDDEN: 해당 보드에 접근할 수 없습니다.
                """, content = @Content),
    ...
    public ResponseEntity<ApiResponseDto<BoardDetailResponseDto>> getBoardDetail(
        ...
        return ResponseEntity.status(HttpStatus.OK)
                .body(ApiResponseDto.success(ResponseCode.BOARD_FETCH_SUCCESS, response));
    }
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "403", description = """
                권한 없음:
                - BOARD_DELETE_FORBIDDEN: 해당 보드를 삭제할 권한 없음
                """, content = @Content),
    ...
    public ResponseEntity<ApiResponseDto<Void>> deleteBoard(
        ...
        return ResponseEntity.status(HttpStatus.OK)
                .body(ApiResponseDto.success(ResponseCode.BOARD_DELETE_SUCCESS, null));
    }
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "스티커 추가 성공"),
            ...
            @ApiResponse(responseCode = "403", description = """
                권한 없음:
                - BOARD_UPDATE_FORBIDDEN: 보드 수정 권한 없음
                """, content = @Content),

            @ApiResponse(responseCode = "404", description = """
                리소스를 찾을 수 없습니다:
                - BOARD_NOT_FOUND
                - BOARD_STICKER_NOT_FOUND
                """, content = @Content)
    })
    ...
    @ApiErrorCodeExamples({
            ...
            ErrorCode.UNAUTHORIZED_USER,
            ErrorCode.BOARD_UPDATE_FORBIDDEN,
            ErrorCode.BOARD_STICKER_MAP_NOT_FOUND,
            ErrorCode.BOARD_NOT_FOUND
    })
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "스티커 삭제 성공"),
            ...
            @ApiResponse(responseCode = "404", description = """
                리소스를 찾을 수 없습니다:
                - BOARD_STICKER_MAP_NOT_FOUND: 해당 스티커가 보드에 없음
                - BOARD_NOT_FOUND
                """, content = @Content)
    })
    ...
    public ResponseEntity<ApiResponseDto<Void>> deleteBoardSticker(
        ...
        return ResponseEntity.status(HttpStatus.OK)
                .body(ApiResponseDto.success(ResponseCode.SUCCESS, null));
    }
